...
- **Recoverable midpoint-MPO numerics:** the isolated worker bypasses Quimb's
  process-killing path by sending the measured risky matrix sizes to a
  persistent killable native-SVD service. A service failure enters the
  Quimb-compatible SciPy `gesvd` fallback in a fresh child process; every
  service restart, scaled retry, and eigensolver fallback is reported.
