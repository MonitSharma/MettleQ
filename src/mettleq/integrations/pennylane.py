"""Native PennyLane device backed by MettleQ and Apple GPUs."""
...
try:
    ...
    raise ImportError(
        "The MettleQ PennyLane device requires the optional PennyLane "
...
@simulator_tracking
@single_tape_support
class MettleQDevice(PennyLaneDevice):
    """PennyLane device using MettleQ's Apple-native simulation methods."""
    ...
    def __init__(
        ...
        *,
        ...
        if wires is None:
            raise DeviceError("MettleQDevice requires an explicit wire set")
        super().__init__(wires=wires, shots=shots)
        if self.wires is None or len(self.wires) <= 0:
            raise DeviceError("MettleQDevice requires at least one wire")
